...
static inline int list_empty(struct dlist *list) { return list->right == list; }

#define list_get_first(listhead) ((listhead)->right)

#define list_get_right(_entry_) ((_entry_)->right)
...
#define list_size(n, listhead)                                                 \
  for (struct dlist *entry = (listhead)->right; entry != (listhead);           \
       entry = entry->right, ++n)
